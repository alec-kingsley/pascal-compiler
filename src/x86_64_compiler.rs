...
fn evaluate_constant(label_idx: &mut u32, rodata: &mut Vec<(u32, String)>, value: String, tipe: &Type) -> String {
    match tipe {
...
fn evaluate_type(tipe1: Type, tipe2: Type) -> Type {
    if tipe1 == tipe2 {
        tipe1
    } else if (tipe1 == Type::Integer && tipe2 == Type::Real) || (tipe1 == Type::Real && tipe2 == Type::Integer) {
        Type::Real
    } else if (tipe1 == Type::Integer && tipe2 == Type::Char) || (tipe1 == Type::Char && tipe2 == Type::Integer) {
        Type::Char
    } else {
        Type::Undefined
    }
...
fn evaluate_factor(factor: &Factor, src: &str, label_idx: &mut u32, errors: &mut u32, warnings: &mut u32, rodata: &mut Vec<(u32, String)>, variable_map: &HashMap<String, (isize, Type)>, constant_map: &HashMap<String, (String, Type)>) -> (String, Type, bool) {
  if let Factor::Constant(unsigned_constant) = factor {
        if let UnsignedConstant::UnsignedInteger(n) = unsigned_constant {
            ...
            report(src, *start, *start + "NIL".len(), "Invalid value in expression", "error");
            *errors += 1;
        ...
        evaluate_expression(expression, src, label_idx, errors, warnings, rodata, variable_map, constant_map)
  } else if let Factor::NegatedFactor(sub_factor, start, end) = factor {
        let (value, tipe, is_constant) = evaluate_factor(sub_factor, src, label_idx, errors, warnings, rodata, variable_map, constant_map);
        if is_constant {
            match tipe {
                Type::Boolean => (format!("{}", !value.parse::<bool>().unwrap()), Type::Boolean, true),
                Type::Integer => (format!("{}", !value.parse::<i64>().unwrap()), Type::Integer, true),
                _ => {
        ...
        if name == "ORD" {
            ...
            let (char_value, tipe, is_constant) = evaluate_expression(arguments.get(0).unwrap(), src, label_idx, errors, warnings, rodata, variable_map, constant_map);
            ...
            let (integer_value, tipe, is_constant) = evaluate_expression(arguments.get(0).unwrap(), src, label_idx, errors, warnings, rodata, variable_map, constant_map);
            ...
            if is_constant {
                ...
                match tipe {
                    Type::Boolean | Type::Char => (format!("\tmovb\t{}, %al\n", location), tipe.clone(), false),
                    Type::Integer | Type::Stryng => (format!("\tmovq\t{}, %rax\n", location), tipe.clone(), false),
                    Type::Real => (format!("\tmovsd\t{}, %xmm0\n", location), tipe.clone(), false),
                    _ => { 
        ...
        let (location, array_tipe) = variable_map.get(name).expect(&format!("Unrecognized identifier: {}", name));
        if let Type::Array(sub_tipe, _, _) = array_tipe {
            let (index_value, expected_integer) = evaluate_final_expression(index, src, label_idx, errors, warnings, rodata, variable_map, constant_map);
            if expected_integer != Type::Integer {
                report(src, *start, *end, "Arrays must indexed with integer type", "error");
                *errors += 1;
            }
            match **sub_tipe {
                Type::Boolean | Type::Char => (format!("{}\
                                                    \tmovq\t%rax, %rdx\n\
                                                    \tmovb\t-{}(%rbp, %rdx, 1), %al\n\
                                                    ", index_value, location), 
                                                *sub_tipe.clone(), false),
                Type::Integer | Type::Stryng => (format!("{}\
                                                    \tmovq\t%rax, %rdx\n\
                                                    \tmovq\t-{}(%rbp, %rdx, 8), %rax\n\
                                                    ", index_value, location), 
                                                *sub_tipe.clone(), false),
                Type::Real => (format!("{}\
                                    ...
                                    ", index_value, location), *sub_tipe.clone(), false),
                _ => {
                    report(src, *start, *end, "Unsupported type used", "error");
                    *errors += 1;
                    (String::new(), Type::Undefined, false)
            ...
            (format!("{}\
                ...
                \tdecq\t%rax\n\
                \tmovzbl\t(%rax), %eax\n\
                     ",index_value,location), Type::Char, false)
...
// may use rax, rdx, rcx
fn evaluate_term(term: &Term, src: &str, label_idx: &mut u32, errors: &mut u32, warnings: &mut u32, rodata: &mut Vec<(u32, String)>, variable_map: &HashMap<String, (isize, Type)>, constant_map: &HashMap<String, (String, Type)>) -> (String, Type, bool) {

    let (mut value1, mut tipe1, mut is_constant1) = evaluate_factor(&term.operands[0].clone(), src, label_idx, errors, warnings, rodata, variable_map, constant_map);
        ...
        if term_tipe == Type::Undefined && tipe1 != Type::Undefined && tipe2 != Type::Undefined {
            ...
            *errors += 1;
            ...
            let res = match term_tipe {
                ...
                Type::Real if operator == "/" => format!("{}",value1.parse::<f64>().unwrap() / value2.parse::<f64>().unwrap()),
                Type::Integer if operator == "MOD" => format!("{}",value1.parse::<i64>().unwrap() % value2.parse::<i64>().unwrap()),
            ...
            match operator.as_str() {
                ...
                _ => report(src, term.start, term.end, "Unrecognized operation in term", "error"),
...
fn evaluate_simple_expression(simple_expression: &SimpleExpression, src: &str, label_idx: &mut u32, errors: &mut u32, warnings: &mut u32, rodata: &mut Vec<(u32, String)>, variable_map: &HashMap<String, (isize, Type)>, constant_map: &HashMap<String, (String, Type)>) -> (String, Type, bool) {
    ...
    while operators_idx < simple_expression.operators.len() {
        ...
        let (mut value2, tipe2, is_constant2) = evaluate_term(&simple_expression.operands[operators_idx + 1].clone(), src, label_idx, errors, warnings, rodata, variable_map, constant_map);
        let simple_expression_tipe = evaluate_type(tipe1.clone(), tipe2.clone());
        if simple_expression_tipe == Type::Undefined && tipe1 != Type::Undefined && tipe2 != Type::Undefined {
            report(src, simple_expression.start, simple_expression.end, "Mismatched types in term", "error");
            *errors += 1;
            (value1, tipe1, is_constant1) = (String::new(), Type::Undefined, true);
        } else if is_constant1 && is_constant2 {
            // evaluate constant
            ...
            let res = match simple_expression_tipe {
                ...
                Type::Integer if operator == "-" => format!("{}",value1.parse::<i64>().unwrap() - value2.parse::<i64>().unwrap()),
                Type::Real if operator == "-" => format!("{}",value1.parse::<f64>().unwrap() - value2.parse::<f64>().unwrap()),
                Type::Integer if operator == "OR" => format!("{}",value1.parse::<i64>().unwrap() | value2.parse::<i64>().unwrap()), 
                Type::Boolean if operator == "OR" => format!("{}",value1.parse::<bool>().unwrap() || value2.parse::<bool>().unwrap()), 
                _ => String::new(),
            };
            ...
            // preform the operation
            match operator.as_str() {
                "+" if simple_expression_tipe == Type::Integer => out.push_str("\taddq\t%rdx, %rax\n"),
                "+" if simple_expression_tipe == Type::Real => out.push_str("\taddsd\t%xmm1, %xmm0\n"),
                "+" if simple_expression_tipe == Type::Char => out.push_str("\taddb\t%dl, %al\n"),
                // TODO - Implement + for type String
                "-" if simple_expression_tipe == Type::Integer => out.push_str("\tsubq\t%rdx, %rax\n"),
                "-" if simple_expression_tipe == Type::Real => out.push_str("\tsubsd\t%xmm1, %xmm0\n"),
                "-" if simple_expression_tipe == Type::Char => out.push_str("\tsubb\t%dl, %al\n"),
                "OR" if simple_expression_tipe == Type::Integer => out.push_str("\torq\t%rdx, %rax\n"),
                "OR" if simple_expression_tipe == Type::Boolean => out.push_str("\torb\t%dl, %al\n"),
                _ if simple_expression_tipe == Type::Undefined => {}, // already handled
                _ => {
                    report(src, simple_expression.start, simple_expression.end, "Unrecognized operation", "error");
                    *errors += 1;
                },
            } 
...
fn evaluate_expression(expression: &Expression, src: &str, label_idx: &mut u32, errors: &mut u32, warnings: &mut u32, rodata: &mut Vec<(u32, String)>, variable_map: &HashMap<String, (isize, Type)>, constant_map: &HashMap<String, (String, Type)>) -> (String, Type, bool) {

    let (mut value1, tipe1, is_constant1) = evaluate_simple_expression(&expression.operand1, src, label_idx, errors, warnings, rodata, variable_map, constant_map);

    if &expression.operator != "NONE" { 
        let (mut value2, tipe2, is_constant2) = evaluate_simple_expression(&expression.operand2, src, label_idx, errors, warnings, rodata, variable_map, constant_map);
        ...
        if is_constant1 && is_constant2 {
            let out = if expression_tipe == Type::Real {
                match expression.operator.as_str() {
                    ...
                    ">=" => format!("{}", value1.parse::<f64>().unwrap() >= value2.parse::<f64>().unwrap()),
                    _   => {
                        report(src, expression.start, expression.end, "Unrecognized operator", "error");
                        *errors += 1;
                        String::new()
                    },
                }
                ...
                match expression.operator.as_str() {
                    "<"  => format!("{}", value1.parse::<i64>().unwrap() < value2.parse::<i64>().unwrap()), 
                    "<=" => format!("{}", value1.parse::<i64>().unwrap() <= value2.parse::<i64>().unwrap()),
                    "="  => format!("{}", value1.parse::<i64>().unwrap() == value2.parse::<i64>().unwrap()),
                    "<>" => format!("{}", value1.parse::<i64>().unwrap() != value2.parse::<i64>().unwrap()),
                    ">"  => format!("{}", value1.parse::<i64>().unwrap() > value2.parse::<i64>().unwrap()),
                ...
                match expression.operator.as_str() {
                    "<"  => format!("{}", value1.parse::<u8>().unwrap() < value2.parse::<u8>().unwrap()), 
                    "<=" => format!("{}", value1.parse::<u8>().unwrap() <= value2.parse::<u8>().unwrap()),
                    "="  => format!("{}", value1.parse::<u8>().unwrap() == value2.parse::<u8>().unwrap()),
                    "<>" => format!("{}", value1.parse::<u8>().unwrap() != value2.parse::<u8>().unwrap()),
                    ">"  => format!("{}", value1.parse::<u8>().unwrap() > value2.parse::<u8>().unwrap()),
            ...
            if expression_tipe == Type::Undefined && tipe1 != Type::Undefined && tipe2 != Type::Undefined {
                report(src, expression.start, expression.end, "Mismatched types in expression", "error");
                *errors += 1;
            }
            out.push_str(&value2);
            if tipe1 == Type::Real {
...
fn evaluate_final_expression(expression: &Expression, src: &str, label_idx: &mut u32, errors: &mut u32, warnings: &mut u32, rodata: &mut Vec<(u32, String)>, variable_map: &HashMap<String, (isize, Type)>, constant_map: &HashMap<String, (String, Type)>) -> (String, Type) {
    let (value, tipe, is_constant) = evaluate_expression(expression, src, label_idx, errors, warnings, rodata, variable_map, constant_map);
    if is_constant {
...
fn process_block(code: &Block, src: &str, label_idx: &mut u32, errors: &mut u32, warnings: &mut u32, rodata: &mut Vec<(u32, String)>) -> String {
    let mut out = String::new();
   
    let mut stack_offset = 0;
    let constant_map = get_constant_map(&code.constants, src, label_idx, errors, warnings);
    for variable in &code.local_variables {
        let tipe = convert_supertype_to_type(&variable.tipe, src, label_idx, errors, warnings, rodata, &constant_map); 
    ...
    let variable_map = get_variable_map(&code.local_variables, src, label_idx, errors, warnings, rodata, &constant_map);
    ...
    if let Statement::StatementList(ref statements) = &code.body {
        for statement in statements {
            out.push_str(&process_statement(statement, src, label_idx, errors, warnings, rodata, &variable_map, &constant_map));
        }
    } else {
...
fn convert_supertype_to_type(super_type: &SuperType, src: &str, label_idx: &mut u32, errors: &mut u32, warnings: &mut u32, rodata: &mut Vec<(u32, String)>, constant_map: &HashMap<String, (String, Type)>) -> Type {
    match super_type {
        ...
        SuperType::Array(element_type, start_expr, end_expr) => {
            let (start_index, _, _) = evaluate_expression(start_expr, src, label_idx, errors, warnings, rodata, &HashMap::new(), constant_map);
            let (end_index, _, _) = evaluate_expression(end_expr, src, label_idx, errors, warnings, rodata, &HashMap::new(), constant_map);
            let converted_element_type = convert_supertype_to_type(element_type, src, label_idx, errors, warnings, rodata, constant_map);
            // TODO - Add useful error messages (each must be CONSTANT and INTEGERS)
            Type::Array(Box::new(converted_element_type), start_index.parse::<isize>().expect("Invalid start idx"), end_index.parse::<isize>().expect("Invalid end idx"))
        }
    }
}

// returns map of symbol names to their %rbp offsets/types
fn get_variable_map(variables: &Vec<Variable>, src: &str, label_idx: &mut u32, errors: &mut u32, warnings: &mut u32, rodata: &mut Vec<(u32, String)>, constant_map: &HashMap<String, (String, Type)>) -> HashMap<String, (isize, Type)> {
    ...
    for variable in variables {
        ...
        result.insert(variable.name.clone(), (stack_offset + stack_offset_offset, tipe.clone()));
    }
    result
}

fn get_constant_map(constants: &Vec<Constant>, src: &str, label_idx: &mut u32, errors: &mut u32, warnings: &mut u32) -> HashMap<String, (String, Type)> {
    let mut result = HashMap::new();
    ...
    for constant in constants {
            let (value, tipe, _) = evaluate_expression(&constant.value, src, label_idx, errors, warnings, &mut Vec::new(), &HashMap::new(), &result);
...
fn process_statement(code: &Statement, src: &str, label_idx: &mut u32, errors: &mut u32, warnings: &mut u32, rodata: &mut Vec<(u32, String)>,  variable_map: &HashMap<String, (isize, Type)>, constant_map: &HashMap<String, (String, Type)>) -> String {
    let mut out = String::new();
    if let Statement::Assignment(name, expression, start, end) = code {
        let (value, tipe1) = evaluate_final_expression(expression, src, label_idx, errors, warnings, rodata, variable_map, constant_map);
        out.push_str(&value);
        let (location, tipe2) = variable_map.get(name).expect(&format!("Unrecognized identifier: {}", name));
        if tipe1 != *tipe2 &&
        ...
        out.push_str(&match tipe2 {
            ...
            Type::Stryng if tipe1 == Type::Char => format!("\tmovb\t%al, -{}(%rbp)\n\tmovb\t$0, -{}(%rbp)\n", location, location + 1),
            Type::Integer => format!("\tmovq\t%rax, -{}(%rbp)\n", location),
            Type::Real if tipe1 == Type::Real => format!("\tmovq\t%xmm0, -{}(%rbp)\n", location),
            Type::Real if tipe1 == Type::Integer => format!("\tcvtsi2sd %rax, %xmm0\n\tmovq\t%xmm0, -{}(%rbp)\n", location),
        ...
        let (index_value, expected_integer) = evaluate_final_expression(index, src, label_idx, errors, warnings, rodata, variable_map, constant_map);
        if expected_integer != Type::Integer {
            report(src, *start, *end, "Expected integer to access array element", "error");
            *errors += 1;
        }
        let (location, arr_tipe) = variable_map.get(name).expect(&format!("Unrecognized identifier: {}", name));
        if let Type::Array(sub_tipe, _, _) = arr_tipe {
            out.push_str(&index_value);
            out.push_str("\tpushq\t%rax\n");
            let (value, tipe) = evaluate_final_expression(expression, src, label_idx, errors, warnings, rodata, variable_map, constant_map);
            ...
            out.push_str(&match **sub_tipe {
                Type::Char | Type::Boolean => format!("\tmovb\t%al, -{}(%rbp, %rdx, 1)\n", location),
                Type::Stryng | Type::Integer => format!("\tmovq\t%rax, -{}(%rbp, %rdx, 8)\n", location),
                Type::Real if tipe == Type::Real => format!("\tmovq\t%xmm0, -{}(%rbp, %rdx, 8)\n", location),
                Type::Real if tipe == Type::Integer => format!("\tcvtsi2sd %rax, %xmm0\n\tmovq\t%xmm0, -{}(%rbp, %rdx, 8)\n", location),
                _ => panic!("Unsupported type used in assignment"),
            });

        } else {
            report(src, *start, *end, "Identifier does not belong to an array", "error");
            *errors += 1;
        }
        ...
        if name == "WRITELN" || name == "WRITE" {
            ...
            for i in 0..arguments.len() {
                let (mut value, tipe, is_constant) = evaluate_expression(&arguments[i], src, label_idx, errors, warnings, rodata, variable_map, constant_map);
                ...
                match tipe {
                    Type::Integer => {
                        let label = request_label(label_idx, rodata, &format!(".string \"{}\"", if new_line {"%ld\\n"} else {"%ld"}));
                        out.push_str(&format!("\
                            \tmovq\t%rax, %rsi\n\
                            \tleaq\tl{}(%rip), %rdi\n\
                            \tmovq\t$0, %rax\n\
                    ...
                    Type::Real => {
                        let label = request_label(label_idx, rodata, &format!(".string \"{}\"", if new_line {"%lf\\n"} else {"%lf"}));
                    ...
                    Type::Stryng if !is_constant => {
                        let label = request_label(label_idx, rodata, &format!(".string \"{}\"", if new_line {"%s\\n"} else {"%s"}));
                        out.push_str(&format!("\
                            \tmovq\t%rax, %rsi\n\
                            \tleaq\tl{}(%rip), %rdi\n\
                            \tmovq\t$0, %rax\n\
                    ...
                    Type::Stryng if is_constant => {
                        let label = request_label(label_idx, rodata, &format!(".string \"{}\"", 
                                        if new_line {format!("{}\\n",&value)} else {format!("{}",&value)}));
                        out.push_str(&format!("\
                            \tleaq\tl{}(%rip), %rdi\n\
                            \tmovq\t$0, %rax\n\
                            \tcall\tprintf\n", label));
                    ...
                    Type::Char => {
                        let label = request_label(label_idx, rodata, &format!(".string \"{}\"", if new_line {"%c\\n"} else {"%c"}));
                        out.push_str(&format!("\
                            \tmovb\t%al, %sil\n\
                            \tleaq\tl{}(%rip), %rdi\n\
                            \tmovq\t$0, %rax\n\
                    ...
                    Type::Boolean => {
                        let label_false = request_label(label_idx, rodata, &format!(".string \"{}\"", if new_line {"FALSE\\n"} else {"FALSE"}));
                        let label_true = request_label(label_idx, rodata, &format!(".string \"{}\"", if new_line {"TRUE\\n"} else {"TRUE"}));
                    ...
                    _ => {
                        report(src, *start, *end, "Print function not defined for all types in call", "error");
                        *errors += 1;
                    },
        ...
        for var in vars {
            let (location, tipe) = variable_map.get(var).expect(&format!("Unrecognized identifier: {}", var));
            match tipe {
                Type::Char => {
                    let l1 = *label_idx;
                    *label_idx += 1;
    ...
    } else if let Statement::IfStatement(condition, true_body, false_body, condition_start, condition_end) = code {
        let has_else = !matches!(**false_body, Statement::DoNothing);
        let (value, tipe) = evaluate_final_expression(condition, src, label_idx, errors, warnings, rodata, variable_map, constant_map);
        if tipe != Type::Boolean && tipe != Type::Undefined {
            report(src, *condition_start, *condition_end, "Condition must be a boolean type", "error");
            *errors += 1;
        }
        out.push_str(&value);
        let l1 = *label_idx;
        *label_idx += 1;
        ...
        out.push_str(&format!("\
                     ...
                     \tje\tl{}\n\
                     ", l1));
        out.push_str(&process_statement(true_body, src, label_idx, errors, warnings, rodata, variable_map, constant_map));
        if has_else {
            l2 = *label_idx;
            *label_idx += 1;
        ...
        if has_else {
            out.push_str(&process_statement(false_body, src, label_idx, errors, warnings, rodata, variable_map, constant_map));
            out.push_str(&format!("l{}:\n", l2));
        }
        ...
        *label_idx += 1;
        out.push_str(&format!("\
                    \ttestb\t%al, %al\n\
                    \tje\tl{}\n", l2));
        out.push_str(&process_statement(body, src, label_idx, errors, warnings, rodata, variable_map, constant_map));
        out.push_str(&format!("\
                    \tjmp\tl{}\n\
                    l{}:\n\
                    ", l1, l2));
        ...
        *label_idx += 1;
        ...
        out.push_str(&format!("\
                    \ttestb\t%al, %al\n\
                    \tje\tl{}\n", l1));
    } else if let Statement::ForLoop(name, name_start, name_end, start, end, range_start, range_end, ascending, body) = code {
        let (location, tipe) = variable_map.get(name).expect(&format!("Unrecognized identifier: {}", name));
        if *tipe != Type::Integer {
            report(src, *name_start, *name_end, "For loop iterator must be integer type", "error");
            *errors += 1;
        }
        let (start_value, start_tipe) = evaluate_final_expression(start, src, label_idx, errors, warnings, rodata, variable_map, constant_map);
        let (end_value, end_tipe) = evaluate_final_expression(end, src, label_idx, errors, warnings, rodata, variable_map, constant_map);
        if start_tipe != Type::Integer || end_tipe != Type::Integer {
            report(src, *range_start, *range_end, "For loop range must consist of integers", "error");
            *errors += 1;
        }
...
pub fn compile(code: Program, src: &str) -> (String, u32, u32) {
    ...
    let mut errors = 0;
    let mut warnings = 0;
    let body = process_block(&code.body, src, &mut label_idx, &mut errors, &mut warnings, &mut rodata);
